function generate_traindataset1d(network::AbstractNetworkTyp, training_setup::Traindata_Settings)
    ...
    @timeit to "loop over meshes" for mesh in meshes
        ...
        for r in polydegrees 
            ...
            #loop over functions
            @timeit to "loop over functions" for func in 1:26
            ...
            @timeit to "loop troubled cells" for t in 1:1100
                ul = rand(Uniform(-1,1))
                ur = rand(Uniform(-1,1))
    ...
    @timeit to "save Dataset" h5open("datasets/1d/traindata1d$(typeof(network)).h5", "w") do file
