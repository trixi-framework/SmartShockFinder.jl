...
function Output1d(func, xi, h)
    if func <= 18
      Y = [0; 1]
    elseif func > 18 && func <=26
      if 0 >= xi-(1/2)*h && 0 <= xi+(1/2)*h
        Y = [1; 0]
      else
        Y = [0; 1]
      end
...
function troubledcellfunctionstep1d(x, ul, ur, x0)
    u = ul*(x<=x0) + ur*(x>x0)
    return u
end

function good_cell1d(node_coord, h, x0)
    x = node_coord[1] + h/2
    if x0 < x+(1/2)*h && x0 > x-(1/2)*h
        return false
    else
        return true
    end
