...
function generate_validdataset1d(datatyp, n_meshes, polydeg, data_input_size)
    n_dims = 1
    #datatyp = "NNPP"
    ...
    println("Safe data")
    h5open("datasets/$(n_dims)d/validdata1d$(datatyp).h5", "w") do file
        write(file, "X", X)
        write(file, "Y", Y)
    end
    return size(X)
